...
export const SelectRadio = ({
  ...
  const hanleSearchItems = (e: FormEvent<HTMLInputElement>) => {
    ...
    setItemList(
      valueList.filter((i) =>
        i.toString().toLowerCase().includes(q.toLowerCase())
      )
    );
  };
  return (
    <div className="items-center my-4">
      ...
      <div className="md:max-w-[350px] w-full flex flex-col relative">
        <button
          id={label.trim().toLowerCase().replace(/\s/g, "-")}
          className={`xs:min-w-[200px] flex items-center justify-between w-full border-2 border-[#cae0e7] pl-3 pr-8 py-2 outline-none focus:shadow-3xl focus:border-primary ${
            errors && "bg-red-200"
          }`}
          type="button"
          onClick={(e) => {
      ...
      {errors && (
        <div className="flex justify-left mt-1 text-sm ">
          <p className={"block  text-red-500 font-medium"}>{errors?.message}</p>
        </div>
      )}
    </div>
  );
